...
function my_woocommerce_get_regular_price($price, $_product) {
    ...
    $two_procent = ($price * $kurs_cb['kurs'])/100*9;
    $new_regular_price_bez = $price * $kurs_cb['kurs'] ;

    $new_regular_price = $two_procent + $new_regular_price_bez;


    return ceil($new_regular_price/50) * 50; // новая цена
    }
...
function my_woocommerce_get_price($price, $_product) {
    ...
    $kurs_current = ($kurs_cb['kurs'] <= 0) ? $kurs_current : $kurs_cb['kurs'];
    $two_procent = ($price * $kurs_cb['kurs'])/100*9 ;
    $new_price_bez = $price * $kurs_cb['kurs'] ;

    $new_price = $two_procent + $new_price_bez;


    return ceil($new_price/50) * 50;; // новая цена
}
function get_currency_cb($code_valute = 'USD', $time_cash = '300' ) {
    // параметры -------------------;
    if ($code_valute != 'USD' && $code_valute != 'EUR') $code_valute = 'USD';
    if ($time_cash <= 0) $time_cash        = 300;         // время кеширования в минутах
    // ------------------------------
